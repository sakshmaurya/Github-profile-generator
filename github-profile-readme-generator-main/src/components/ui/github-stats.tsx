      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            console.warn('GitHub repo not found. Update YOUR_USERNAME/YOUR_REPO in github-stats.tsx');
            setError(true);
            setIsLoading(false);
            return;
          }
  ...
  return (
    ...
    >
      ...
      <motion.a
        ...
        title="Star YOUR_USERNAME/YOUR_REPO on GitHub"
      ...
      <motion.a
        ...
        rel="noopener noreferrer"
        className="border-foreground bg-muted hover:bg-accent flex items-center gap-1.5 border-2 px-3 py-1.5 text-xs font-medium transition-colors"
        title="Fork YOUR_USERNAME/YOUR_REPO on GitHub"
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
      >
        <motion.svg
